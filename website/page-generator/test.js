...
describe('Generate pages', () => {
    ...
    beforeAll(async () => {
        const packagesCwd = await copyFixtureIntoTempDir(__dirname, 'simple-mock-packages');
    ...
    describe('package pages generation', () => {
        it('creates a home page per package', () => {
            expect(
                fs.existsSync(path.join(pagesPath, 'packages', 'mock-package1', 'index.js')),
            ).toEqual(true);
            expect(
                fs.existsSync(path.join(pagesPath, 'packages', 'mock-package2', 'index.js')),
            ).toEqual(true);
            expect(
                fs.existsSync(path.join(pagesPath, 'packages', 'mock-package2', 'index.js')),
            ).toEqual(true);
        ...
        it('creates docs pages for each package', () => {
            const assertDocs = packageId => {
                const packageDocsPath = path.join(pagesPath, 'packages', packageId, 'docs');
                expect(fs.existsSync(path.join(packageDocsPath, 'index.js'))).toEqual(true);
                expect(fs.existsSync(path.join(packageDocsPath, 'extended-info.js'))).toEqual(true);
                expect(fs.existsSync(path.join(packageDocsPath, 'special-usecase.js'))).toEqual(
                    true,
                );
            };
        ...
        it('creates examples pages for each package', () => {
            const assertDocs = packageId => {
                const packageExamplesPath = path.join(pagesPath, 'packages', packageId, 'examples');
                expect(fs.existsSync(path.join(packageExamplesPath, 'index.js'))).toEqual(true);
                expect(fs.existsSync(path.join(packageExamplesPath, 'example1.js'))).toEqual(true);
                expect(fs.existsSync(path.join(packageExamplesPath, 'example2.js'))).toEqual(true);
                expect(fs.existsSync(path.join(packageExamplesPath, 'example3.js'))).toEqual(true);
            };
        ...
        describe('sitemap generation', () => {
            ...
            it('gets the examples pages for each package', () => {
                const assertDocs = (examples, packageId) => {
                    const packageDocsPath = path.join('/', 'packages', packageId, 'examples');
            ...
            it('gets the docs folder path for each package', () => {
                expect(packagesSitemap[0].docPath).toEqual('/packages/mock-package1/docs');
                expect(packagesSitemap[1].docPath).toEqual('/packages/mock-package2/docs');
                expect(packagesSitemap[2].docPath).toEqual('/packages/mock-package3/docs');
            });

            it('gets the examples folder path for each package', () => {
                expect(packagesSitemap[0].examplePath).toEqual('/packages/mock-package1/examples');
                expect(packagesSitemap[1].examplePath).toEqual('/packages/mock-package2/examples');
                expect(packagesSitemap[2].examplePath).toEqual('/packages/mock-package3/examples');
            });
        });
    });
    ...
    describe('docs pages generation', () => {
        it('creates pages for each markdown file', () => {
            expect(fs.existsSync(path.join(pagesPath, 'docs', 'doc-1.js'))).toEqual(true);
            expect(fs.existsSync(path.join(pagesPath, 'docs', 'doc-2.js'))).toEqual(true);
            expect(fs.existsSync(path.join(pagesPath, 'docs', 'doc-3', 'doc-3-1.js'))).toEqual(
                true,
            );
            expect(
                fs.existsSync(path.join(pagesPath, 'docs', 'doc-3', 'doc-3-2', 'doc-3-2-1.js')),
            ).toEqual(true);
        });
        ...
        describe('sitemap generation', () => {
            ...
            it('gets url paths for all docs pages in the filesystem structure', () => {
                ...
                // nested docs
                expect(docsSitemap[2].children[0].pagePath).toEqual('/docs/doc-3/doc-3-1');
...
describe('File modification tests', () => {
    ...
    it('should remove files from package docs pages that are removed from disc on rerun', async () => {
        ...
        generatePages([path.join(packagesPath, '/*')], docsPath, pagesPath, componentsPath);

        expect(fs.existsSync(firstDocsPage)).toEqual(true);

        fs.unlinkSync(path.join(packagesPath, 'mock-package1', 'docs', 'extended-info.md'));
        generatePages([path.join(packagesPath, '/*')], docsPath, pagesPath, componentsPath);

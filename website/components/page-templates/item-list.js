...
const ItemList = ({ data, type }) => {
    const getRows = () => {
        const packagePages = pageInfo.packages.find(pkg => pkg.packageId === data.id);
        return packagePages[type].map(item => renderRow(item));
    };
    const getDocsList = () => (
        <Page>
            <Title>{type === 'docs' ? 'Document Home Page' : 'Example Home Page'}</Title>
            <Section>
                <Table head={head} rows={getRows()} defaultSortKey="name" defaultSortOrder="ASC" />
            </Section>
        </Page>
    );
